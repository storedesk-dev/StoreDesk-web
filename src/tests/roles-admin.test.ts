...
import { GET as list, POST as create } from "@/app/api/v1/admin/stores/[storeId]/roles/route";
import { DELETE as remove, PUT as save } from "@/app/api/v1/admin/stores/[storeId]/roles/[roleId]/route";
...
describe("GET …/roles", () => {
  it("lists every role with its version and user count, and the templates", async () => {
    ...
    const res = await call(list, request("GET", "/", { token: admin.token }), { storeId });
    ...
    expect((await call(list, request("GET", "/"), { storeId })).status).toBe(401);
  });
...
describe("POST …/roles", () => {
  it("creates a role from a template, derives its id, audits and notifies", async () => {
    const res = await call(create, request("POST", "/", { token: admin.token, body: { roleName: "Shift Lead", template: "store_manager" } }), { storeId });
    ...
    expect(scheduleNotify).toHaveBeenCalledWith({ storeId, reason: "role.create" });

    const twin = await call(create, request("POST", "/", { token: admin.token, body: { roleName: "Shift Lead", template: "blank" } }), { storeId });
  ...
  it("uses the access the editor sends, and 409 for an id in use", async () => {
    ...
    const taken = await call(create, request("POST", "/", { token: admin.token, body: { roleName: "X", roleId: "night" } }), { storeId });
  ...
  it.each([
    ...
    expect((await call(create, request("POST", "/", { token: admin.token, body }), { storeId })).status).toBe(400);
  });
});
...
describe("PUT …/roles/{roleId}", () => {
  ...
  it("saves on the version read, audits and notifies every store", async () => {
    const res = await call(save, request("PUT", "/", { token: admin.token, body: body(1) }), { storeId, roleId: "cashier" });
    expect(res.status).toBe(200);
    ...
    expect(scheduleNotify).toHaveBeenCalledWith({ storeId, reason: "role.update" });
  });

  it("answers 409 ROLE_VERSION_CONFLICT with the current role after a store server saved it", async () => {
    await updateRoleFromEdge(storeId, "cashier", { baseVersion: 1, roleName: "Edited at the store", accessKeys: cashier().accessKeys });
  ...
  it("answers 404 for an unknown role and 400 for a bad shape", async () => {
    expect((await call(save, request("PUT", "/", { token: admin.token, body: body(1) }), { storeId, roleId: "nope" })).status).toBe(404);
    const bad = await call(save, request("PUT", "/", { token: admin.token, body: { roleName: "x", accessKeys: {} } }), { storeId, roleId: "cashier" });
  ...
  it("keeps a page the registry doesn't know, and says so", async () => {
    ...
    const res = await call(save, request("PUT", "/", { token: admin.token, body: { baseVersion: 1, roleName: "Cashier", accessKeys } }), { storeId, roleId: "cashier" });
    expect(res.status).toBe(200);
  ...
  it("keeps a retired page a stored role still has, without a warning", async () => {
    ...
    const listed = await call(list, request("GET", "/", { token: admin.token }), { storeId });
...
describe("DELETE …/roles/{roleId}", () => {
  it("answers 409 ROLE_IN_USE while a user has it, then deletes, audits and notifies", async () => {
    ...
    const inUse = await call(remove, request("DELETE", "/", { token: admin.token }), { storeId, roleId: "viewer" });
    expect(inUse.status).toBe(409);
    expect(inUse.body).toMatchObject({ error: { code: "ROLE_IN_USE" }, userCount: 1 });
    expect(added.user.assignments).toHaveLength(1);

    const res = await call(remove, request("DELETE", "/", { token: admin.token }), { storeId, roleId: "store_manager" });
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ deleted: "store_manager" });
    expect(await lastAudit("role.delete")).toMatchObject({ targetId: "store_manager" });
    expect(scheduleNotify).toHaveBeenCalledWith({ storeId, reason: "role.delete" });
    expect((await call(remove, request("DELETE", "/", { token: admin.token }), { storeId, roleId: "store_manager" })).status).toBe(404);
  });
});
...
describe("the Organization Admin role", () => {
  ...
  async function storeOldAdminRole(version: number) {
    const { TenantStoreModel } = await import("@/models/ControlPlane");
    const store = (await TenantStoreModel.findOne({ storeId }).lean()) as { roles: Array<Record<string, unknown>> };
  ...
  it("reads with every registered page and flag, one version up, and stores that so stores re-sync", async () => {
    const TenantStoreModel = await storeOldAdminRole(3);
    const res = await call(list, request("GET", "/", { token: admin.token }), { storeId });
    ...
    const stored = (await TenantStoreModel.findOne({ storeId }).lean()) as { roles: Array<{ roleId: string; version: number }> };
    expect(stored.roles.find((entry) => entry.roleId === "org_admin")?.version).toBe(4);
    const again = await call(list, request("GET", "/", { token: admin.token }), { storeId });
  ...
  it("never grants new pages to another role", async () => {
    ...
    const roles = store.roles.map((role) => (role.roleId === "viewer" ? { ...role, version: 2, accessKeys: withoutDeals } : role));
    await TenantStoreModel.updateOne({ storeId }, { $set: { roles } });
  ...
  it("keeps every page whatever an edit sends, from the admin or a store", async () => {
    ...
    const edge = await updateRoleFromEdge(storeId, "org_admin", { baseVersion: 2, roleName: "Owner", accessKeys: off });
