...
/**
 * The reusable setup key and Replace PC (owner decision 2026-09-17): the key
 * never expires and activates the store again on this PC or another; the new
 * PC replaces the old one (credential revoked, tunnel rotated); an admin can
 * read it any time (audited) and rotate it; the store's own PC can read it and
 * give its installation up.
 */
...
const edgeKey = (credential: string) => call(edgeSetupKey, request("GET", "/api/v1/edge/setup-key", { token: credential }));
const release = (credential: string, body: unknown = { confirm: "REPLACE_PC" }) =>
...
describe("redeeming a reusable key again", () => {
  it("activates the same installation on a second PC and replaces the first PC", async () => {
    const { body } = await issue();
    const first = await redeemKey(body.setupKey);
    expect(first.status).toBe(201);

    const second = await redeemKey(body.setupKey);
    ...
    expect(second.body.relayKey).not.toBe(first.body.relayKey);
    ...
    expect(await SetupKeyModel.findOne({ keyId: body.keyId }).lean()).toMatchObject({ status: "shown", redeemCount: 2 });

    // The first PC's credential no longer works at the edge.
    expect((await edgeKey(first.body.workerCredential)).status).toBe(401);

    expect(await lastAudit("setup_key.redeem")).toMatchObject({
      metadata: expect.objectContaining({ reusable: true, replacedPc: true, previousCredentialId: first.body.workerCredentialId })
    });
    const audits = JSON.stringify(await AuditEventModel.find({}).lean());
    for (const secret of [body.setupKey, first.body.workerCredential, second.body.workerCredential, second.body.relayKey]) {
      expect(audits).not.toContain(secret);
    }
  });

  it("tells the replaced PC (with its own relay key) and rotates the tunnel so only the new PC gets the token", async () => {
  ...
  it("works again after the admin's Replace PC (the key is not cancelled)", async () => {
    const { body } = await issue();
    ...
    expect(await SetupKeyModel.findOne({ keyId: body.keyId }).lean()).toMatchObject({ status: "shown" });
...
describe("the admin reads and rotates the key", () => {
  ...
  it("rotating stops the old key, keeps the running PC, and the new key is the one revealed", async () => {
    ...
    expect(await lastAudit("setup_key.rotate")).toMatchObject({ metadata: expect.objectContaining({ previousKeyId: body.keyId }) });
    ...
    expect(old.body.error.code).toBe("SETUP_KEY_EXPIRED");
...
describe("the store PC reads its key and gives its installation up", () => {
  it("reads only its own installation's key, audited with the worker as actor", async () => {
    const { body } = await issue();
    const pc = await redeemKey(body.setupKey);

    const own = await edgeKey(pc.body.workerCredential);
    expect(own.status).toBe(200);
    expect(own.body).toEqual({ keyId: body.keyId, setupKey: body.setupKey });
    expect(own.headers.get("Cache-Control")).toContain("no-store");
    expect(await lastAudit("setup_key.reveal")).toMatchObject({ actorType: "worker", actorId: pc.body.workerCredentialId });

    // Another organization's PC gets its own key, never this one.
    ...
    const otherParams = { organizationId: other.organization.organizationId, storeId: other.store.storeId };
    const otherIssued = await call(issueKey, request("POST", "/", { token: otherAdmin.token, body: { deliver: "show" } }), otherParams);
    const otherPc = await redeemKey(otherIssued.body.setupKey);
    const theirs = await edgeKey(otherPc.body.workerCredential);
    expect(theirs.body.setupKey).toBe(otherIssued.body.setupKey);
    expect(theirs.body.setupKey).not.toBe(body.setupKey);
  ...
  it("releases: credential revoked, tunnel rotated, installation awaiting activation; the same key activates again", async () => {
    await giveStoreTunnel();
    const { body } = await issue();
    const pc = await redeemKey(body.setupKey);
    ...
    expect(await lastAudit("installation.release")).toMatchObject({ actorType: "worker", metadata: expect.objectContaining({ tunnelRotated: true }) });
