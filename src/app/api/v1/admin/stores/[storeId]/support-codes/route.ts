...
export async function GET(req: Request, ctx: Ctx) {
  try {
    ...
    return NextResponse.json({ supportCodes: await listSupportCodes(storeId) }, { headers: NO_STORE });
  } catch (error) {
...
export async function POST(req: Request, ctx: Ctx) {
  try {
    ...
    const { storeId } = await ctx.params;
    return NextResponse.json(await issueSupportCode(admin, storeId), { status: 201, headers: NO_STORE });
  } catch (error) {
    return jsonError(error);
  }
}
