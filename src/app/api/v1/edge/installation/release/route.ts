 ...
 * `{confirm: "REPLACE_PC", appUserId?}` → `{released: true, tunnelRotated, reusableKey}`.
 ...
 * Audited `installation.release`, with `appUserId` naming the person who
 * pressed the button rather than only the PC. No secret in the answer.
 */
export async function POST(req: Request) {
  try {
    const worker = await authenticateWorker(req);
    const body = await parseBody(req, ReleaseInstallationSchema);
    return NextResponse.json(await releaseInstallation(worker, body.appUserId), {
